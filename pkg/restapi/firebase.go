...
func findService(uid string, appId string, serviceId string) (*ServiceData, error) {
	...
	token := common.GenerateAuthToken(response.ServiceName + "-" + response.Appname) //+ "." + response.Domain)
	response.Token = token
	response.Endpoint = response.ServiceName + "-" + response.Appname //+ "." + response.Domain
